...
class HomeGermanyActivity : AppCompatActivity() {

    var tabTitle = arrayOf("돌아온 문화재", "잃어버린 문화재")

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        setContentView(R.layout.activity_home_germany)

        var tab = findViewById<TabLayout>(R.id.home_tabLayout)
        var pager = findViewById<ViewPager2>(R.id.home_viewPager2)

        pager.adapter = HomeGermanyViewAdapter(supportFragmentManager, lifecycle)
